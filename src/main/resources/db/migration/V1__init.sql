...
create table executions (finished boolean, created_at timestamp(6) with time zone, execution_pk bigint generated by default as identity, started_at timestamp(6) with time zone, stopped_at timestamp(6) with time zone, wait_till timestamp(6) with time zone, execution_id varchar(255) not null unique, mode varchar(255), process_name varchar(255), retry_of varchar(255), retry_success_id varchar(255), status varchar(255), workflow_id varchar(255), raw_json TEXT, primary key (execution_pk));
...
create table namespaces (namespace_id bigint generated by default as identity, name varchar(120) not null unique, description varchar(255), primary key (namespace_id));
...
create table permission_namespaces (namespace_id bigint not null, permission_id bigint not null, primary key (namespace_id, permission_id));
create table permissions (permission_id bigint generated by default as identity, type varchar(100) not null unique, primary key (permission_id));
...
create table roles (role_id bigint generated by default as identity, name varchar(100) not null unique, description varchar(255), primary key (role_id));
...
create table users (enabled boolean, user_id bigint generated by default as identity, username varchar(120) not null unique, password varchar(255), primary key (user_id));
...
alter table if exists permission_namespaces add constraint FK8kwcalax6mv78wndpxpohpomy foreign key (namespace_id) references namespaces;
alter table if exists permission_namespaces add constraint FKlxmthi7p46o054tyo7m30w8l foreign key (permission_id) references permissions;
