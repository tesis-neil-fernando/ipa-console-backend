...
public class ExecutionsService {
    ...
    public ResponseEntity<N8nApiService.ApiResponse<ExecutionsListResponseDto>> listExecutions(
        ...
        var n8nResp = n8nApiService.getExecutions(includeData, status, workflowId, projectId, limit, cursor);
        ...
        try {
            var envelope = objectMapper.readValue(n8nResp.getBody().getData(), N8nExecutionsEnvelope.class);
            ...
            var err = new N8nApiService.ApiResponse<ExecutionsListResponseDto>(false,
                    "Error al parsear ejecuciones: " + ex.getMessage(), null);
    ...
    private record N8nExecutionsEnvelope(List<N8nExecution> data, String nextCursor) {}
    @JsonIgnoreProperties(ignoreUnknown = true)
    private record N8nExecution(String id, String status, Boolean finished,
